...
public class TestGenericList {
	...
	/**
	 * Creates a generic linked list. Then based on the user's input, adds a
	 * random number of countries to the list.
	 * 
	 * @param allCountries
	 *            An array of Country objects
	 */
	private LinkedList<Country> createRandomListOfCountries(Country[] allCountries) {
		// Prompts the user for the number of countries they want to add to the
		// list.
		...
		// Define the LinkedList class to be a singly list list of generic Node
		// objects.
		...
		for (int i = 0; i < requestedSize; i++) {
			...
			// TODO: Modify all attributes and methods of class LinkedList to
			// hold
			// generic data types.
	...
	/**
	 * Displays the list of countries. Each country index is output along with
	 * the country's index in the list.
	 * 
	 * @param countryList
	...
	private void displayGeneratedList(LinkedList<Country> countryList) {
		...
		// TODO: Implement a nested ListIterator class in your generic
		// LinkedList class
		java.util.Iterator<Country> iterator = countryList.iterator();

		// Uses a reference of type ListIterator to traverse your country list
		// and collect country names
	...
	private void testSearchInList(LinkedList<Country> selectedCountryList) {
		// Tests that the iterator of the LinkedList class by calling its
		// toString() method.
	...
	private void testSubscriptionsTotalInList(LinkedList<Country> selectedCountryList) {
		...
		// System.out.println("Looking at: " + foundCountry); //For Debugging
		...
		try {
			...
			System.out.printf("Total subscriptions for %s between %d and %d is %f \n", foundCountry.getName(),
					requestedStartYear, requestedEndYear, totalSubscriptions);
		} catch (IllegalArgumentException ex) {
			System.out.println("Years requested out of bounds. Valid years are 1960 - 2014");
		}

	}

	/**
	 * Uses a CSV to parse a CSV file. Adds the data for each country to an
	 * array of Country objects. Adds a random selection of countries to a
	 * generic LinkedList object.
	 */
	public static void main(String[] args) {
		...
		final String FILENAME = "resources/cellular.csv"; // test file with
															// latest set of
															// countries and
															// subscription
															// years
		...
		// In class CSVReader the accessor methods only return values of
		// instance variables.
		...
		for (int countryIndex = 0; countryIndex < countries.length; countryIndex++) {
			...
			// TODO: Initially convert your CountryList to a generic LinkedList
			...
			// NOTE: Once you've verified that your generic LinkedList builds
			// correctly,
			// make sure to comment the line below before submitting.
			// current = new Country(countryNames[countryIndex], numberOfYears);
			// // version 1

			// TODO: Once you are successful in creating a generic LinkedList of
			// countries, create a
			...
			current = new Country(countryNames[countryIndex]); // version 2 and
																// final version
																// of Country
																// constructor
		...
		// Tests the generic LinkedList class by adding a random number of
		// entries
		...
		// System.out.println(selectedCountryList); //For Debugging
		...
		// Personal Testing
		System.out.println("\n*******************Personal Testing*************************");
		System.out.println("Testing First Country");
		application.testSearchInList(selectedCountryList);
		System.out.println("\nTesting Last Country");
		application.testSearchInList(selectedCountryList);
		System.out.println("\nTesting Non-Existing Country");
		application.testSearchInList(selectedCountryList);
		...
		// Personal Testing
		System.out.println("\n*******************Personal Testing*************************");
		System.out.println("Testing Both Limits Out of Bounds (Too Low)");
		application.testSubscriptionsTotalInList(selectedCountryList);
		System.out.println("\nTesting Both Limits Out of Bounds (Too High)");
		application.testSubscriptionsTotalInList(selectedCountryList);
		System.out.println("\nTesting Both Limits (Start > End) ");
		application.testSubscriptionsTotalInList(selectedCountryList);
		System.out.println("\nTesting One Limit Out of Bounds (Too Low)");
		application.testSubscriptionsTotalInList(selectedCountryList);
		System.out.println("\nTesting One Limit Out of Bounds (Too High)");
		application.testSubscriptionsTotalInList(selectedCountryList);
